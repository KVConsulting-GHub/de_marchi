...
import {
  useAllClients, useOnboardingProcess, useChecklist, useDocuments,
...
export default function ClientOnboardingPage({
  ...
  const { process }                        = useOnboardingProcess(clientId);
  const { byFase, progressoPct }           = useChecklist(clientId, process?.id ?? "");
    ...
    return (
      <main className="flex-1 flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="font-medium" style={{ color: "#1E2D52" }}>Cliente não encontrado</p>
  ...
  return (
    <>
      <Header
        ...
        subtitle={`CNPJ: ${formatCNPJ(client.cnpj)} · ${client.tipo === "migrante" ? "Migração" : "Novo cliente"}`}
        actions={
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-xl border border-base-300 p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: "rgba(26,26,46,0.55)" }}>Início</p>
            </div>
          </div>
